...
def interactive_prompt():
    ...
    while user_command != 'exit':
        ...
        if sep_command[-1] == 'barplot':
            x = []
            y = []
            if (sep_command[0] == 'companies' or sep_command[0] == 'countries'):
                ...
                for i in command_res:
                    x.append(i[0])
                    y.append(i[1])
            elif (sep_command[0] == 'bars'):
                if sep_command[3] == 'cocoa':
                    for i in command_res:
                        x.append(i[0])
                        y.append(i[4])
                elif sep_command[3] == 'ratings':
                    for i in command_res:
                        x.append(i[0])
                        y.append(i[3])
            fig = go.Figure(data=go.Bar(x=x, y=y), layout=go.Layout(title="The barplot visualization"))
            fig.show()
